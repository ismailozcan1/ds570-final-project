...
The processed files used by the dashboard are stored in:
...
Load the raw Steam user-game interaction data.
Perform basic data checks and exploratory data analysis.
Clean and prepare the dataset.
Convert interaction-level data into user-level behavioral features.
Scale the behavioral features before clustering.
Use K-Means clustering to discover user segments.
Use elbow method and silhouette score to evaluate the cluster structure.
Use PCA to visualize the clusters in two dimensions.
Use supervised models to reproduce and explain the K-Means cluster labels.
Compare the models with a DummyClassifier baseline.
Use Decision Tree feature importance and confusion matrix for interpretability.
Present the results in an interactive Streamlit dashboard.
Make the project runnable with Docker and Docker Compose.
Notebooks
01_data_loading_and_eda.ipynb
